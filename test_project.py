...
video_input = cv.VideoCapture('C://Users/Saumil/Downloads/car1.mp4')

def extractText(p):
    print("Reached Here <Extract Text>")
    predicted_result = pyt.image_to_string(p, lang ='eng')
    filter_predicted_result = "".join(predicted_result.split()).replace(":", "").replace('%', '').replace('#', '').replace("-", "").replace('“', '').replace('@', '').replace('‘', '').replace(')', '').replace('(', '').replace('|', '').replace('.', '')
    return filter_predicted_result

num = "VehicleEntryTracker"
...
def start_video():
    ...
    if check == True:
            ...
            for c in cnts:
                ...
                if len(approx) == 4:  # Select the contour with 4 corners
                    ...
                    if area > 500 and area < 3000:
                        ...
                        print(num)
                        add_unapproved_btn = tk.Button(window, bg = "blue", fg = "white", width = 20, height = 2, text = "Add to Unapproved", command = unapproved_db.add_to_unapproved(num))
                        add_unapproved_btn.pack(padx = 4, pady = 12)
        ...
        video_label.after(1, start_video)
